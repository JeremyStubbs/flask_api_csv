...
class SpecificUsers(Resource):
    def get(self, id):
        df = pd.read_csv('users.csv', usecols=['name', 'city', 'locations'])
        result = df.to_dict(orient='records')
        filtered_dict = [k for k in result if k['name'] == id]
        return {'data': filtered_dict}, 200  # return data and 200 OK

class Users(Resource):   
    def get(self):
        df = pd.read_csv('users.csv', usecols=['name', 'city', 'locations'])
        result = df.to_dict(orient='records')
        return {'data': result}, 200  # return data and 200 OK   
...
api.add_resource(SpecificUsers, '/users/<id>/')  # add endpoints
